...
const Cart = ({ CartProducts, ClientCheckOut, errors }) => {
  ...
  const [someErrors, setSomeErrors] = useState(null);
  ...
  useEffect(() => {
    errors.length > 0 && errors[0].type === "cart"
      ? setSomeErrors(true)
      : setSomeErrors(false);
    CartProducts.length === 0 ? setEmptyCart(true) : setEmptyCart(false);
  }, [errors, CartProducts]);
  return (
    <Container>
      {emptyCart ? (
        <Message
          error
          header="There is No Item in the Cart "
                      ...
                      <Table celled stackable>
                        ...
                        <Table.Body>
                          <Table.Row>
                            <Table.Cell>
                              <p>
                                u have [{CartProducts.length}] product in your
                                Cart
                              </p>
                              <br />
                              {CartProducts.map((el, i) => (
